...
dependencies {
    if (bootstrap) {
        ...
        compileOnly("com.github.StatuParsek:Parsek:2.3.0")
    }
...
publishing {
    repositories {
        maven {
            name = "parsek-plugin-proxy-path-to-url"
            url = uri("https://maven.pkg.github.com/StatuParsek/parsek-plugin-proxy-path-to-url")
            credentials {
    ...
    publications {
        create<MavenPublication>("shadow") {
            project.extensions.configure<com.github.jengelman.gradle.plugins.shadow.ShadowExtension> {
                artifactId = "parsek-plugin-proxy-path-to-url"
